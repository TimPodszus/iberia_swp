...
public enum CityName {
    ...
    ALBACETE("Albacete", 34), ZARAGOZA("Zaragoza", 25), ALICANTE("Alicante", 32), CUENCA("Cuenca", 35), BARCELONA(
            ...
            "Cartagena",
            33
    ), PALMA_DE_MALLORCA("Palma de Mallorca", 29), TERUEL(
            "Teruel",
            36
    ), GIRONA("Girona", 28),
    ...
    EVORA("Evora", 5), LISBOA("Lisboa", 3), COIMBRA("Coimbra", 2), CACERES("Caceres", 6), ALBUFEIRA("Albufeira",
            ...
            "Porto",
            1
    ), A_CORUNA("A Coruna", 12), SANTIAGO_DE_COMPOSTELA(
    ...
    SAN_SEBASTIAN_DONOSTIA("San Sebastian-Donostia", 22), SANTANDER("Santander", 15), GIJON("Gijon", 14), SORIA("Soria",
            ...
            "Madrid",
            17
    ), HUESCA("Huesca", 24), LEON("Leon", 13), BILBAO_BILBO(
            "Bilbao-Bilbo",
            21
    ...
    /**
     * Retrieves a CityName enum constant by its display name.
     *
     * @param name the display name of the city
     * @return the CityName enum constant with the specified display name, or null if no such constant exists
     */
    public static CityName getCityByName(String name) {
        for (CityName city : CityName.values()) {
            if (city.getDisplayName()
                    .equals(name)) {
                return city;
            }
        }

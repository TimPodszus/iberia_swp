...
public class ChatService extends AbstractService {
    ...
    public void onSendChatRequest(SendChatRequest request) {
        ...
        SentChatMessage message = convertToSendChatMessage(chatMessage);
    ...
    public void onGetChatRequest(GetChatRequest request) {
        ...
        try {
            ...
            List<SentChatMessage> messages = chatMessages.stream()
                                                         .map(this::convertToSendChatMessage)
                                                         .toList();
    ...
    /**
     * Converts an IChatMessage to an SentChatMessage.
     *
     * @param chatMessage the IChatMessage to convert
     * @return the converted SentChatMessage
     */
    private SentChatMessage convertToSendChatMessage(IChatMessage chatMessage) {
        if (chatMessage instanceof PlayerChatMessage playerChatMessage) {
            return new PlayerSentChatMessage(
                    chatMessage.getLobbyId(),
                    playerChatMessage.getSender(),
                    playerChatMessage.getMessage()
            );
        } else {
            return new ServerSentChatMessage(chatMessage.getLobbyId(), chatMessage.getMessage());
        }
    }
